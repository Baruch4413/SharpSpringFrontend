...
export default {
    ...
    created () {
        let ctx = this;
        axios.get(
            '/notes/edit?id=' + this.$route.query.id,
            {
                headers: {
            ...
            try {
                ctx.note_title = response.data.title;
                ctx.note_body = response.data.body;
            } catch (e) {
                console.error(e);
            }
        }).catch(function (error) {
            ctx.$swal('Error', 'There has been an error recovering previously saved data', 'error')
        });
    },
    methods: {
        onSubmit () {
            ...
            axios.post(
                ...
                ctx.$swal('Error', 'There has been an error updating the note', 'error')
            });
        }
    },
};
